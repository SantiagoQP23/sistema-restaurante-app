...
export const getTransactions = async (
  ...
  const response = await restauranteApi.get<TransactionResponse>("/transactions", {
    params: {
...
export const updateTransaction = async (id: number, createTransactionDto: UpdateTransactionDto) => {
...
export const deleteTransaction = async (id: number) => {
  const response = await restauranteApi.delete<Transaction>(`/transactions/${id}`);
  return response.data;
}
