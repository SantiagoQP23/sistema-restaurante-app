...
export const generatePdfIncome = async (income: Income): Promise<ICreatePDF> => {
  ...
  pdf.add(
    pdf.add(
      new Txt(`Transacción N° ${income.transaction.num}`).alignment('center').bold().fontSize(10).margin([0, 10, 0, 10]).end
    )
  );
  ...
  pdf.add(
    new Txt(`Fecha: ${format(new Date(income.createdAt), 'dd MMMM yyyy HH:mm', { locale: es })}`).margin([0, 0, 0, 5]).end

  );

  pdf.add(
    new Txt(`Monto: ${formatMoney(income.transaction.amount)}`).margin([0, 0, 0, 5]).end
  );

  pdf.add(
    new Txt(`Descripción: ${income.transaction.description}`).margin([0, 0, 0, 5]).end
  );
  ...
  pdf.add(
    new Txt(`${income.transaction.user.person.firstName} ${income.transaction.user.person.lastName}`).alignment('center').margin([0, 0, 0, 5]).end
  );
