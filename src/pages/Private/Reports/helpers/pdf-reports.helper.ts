...
export const generateWaiterReportPdf = async (data: ResponseIncomesByUser[], filterDto: DateFilterDto, urlImage?: string): Promise<ICreatePDF> => {
  ...
  if (urlImage) {
    ...
    pdf.add(
      pdf.add(await new Img(urlImage).width(chartWidth).height(chartHeight).build())
