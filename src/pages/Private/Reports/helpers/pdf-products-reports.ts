...
import { BestSellingCategoriesResponse, ResultBestSellingProducts } from '../services/dashboard.service';
...
export const generateProductsReport = async (
  ...
  const categoriesBody = categories.map(category => [category.categoryName, category.totalSold]);

  pdf.add(
    new Txt('Categorías más vendidos').bold().margin([0, 10, 0, 10]).end
  )
  pdf.add(
    new Table([
      categoriesHeader,
      ...categoriesBody
    ]).widths(['*', '*']).layout('lightHorizontalLines').end
  )


  const productsHeader = ['Producto', 'Categoría', 'Cantidad', 'Ingresos'];
  const productsBody = products.map(product => [product.productName, product.categoryName, product.totalSold, formatMoney(product.productPrice * product.totalSold)]);

  pdf.add(
    new Txt('Productos más vendidos').bold().margin([0, 10, 0, 10]).end
  )
