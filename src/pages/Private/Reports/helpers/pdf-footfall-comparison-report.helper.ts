...
export const generateFootfallComparisonReport = async (data: ComparisonFootfallResponse, filters: DateFiltePaginationDto, urlImage?: string): Promise< ICreatePDF> => {
  ...
  PdfMakeWrapper.setFonts(pdfFonts);
    ...
    pdf.add(
      new Txt('Reporte de Comparación de afluencia').alignment('center').bold().fontSize(15).margin([0, 10, 0, 1]).end
    );
    ...
    const header = ['Fecha', 'Afluencia real', 'Afluencia pronosticada', 'Diferencia']
