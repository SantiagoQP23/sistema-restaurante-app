...
export const generateFootfallSimulationReport = async (data: FootfallResponse, filters: DateFiltePaginationDto, urlImage?: string): Promise< ICreatePDF> => {
  ...
  PdfMakeWrapper.setFonts(pdfFonts);
    ...
    pdf.add(
      await new Img(img).width(50).height(50).alignment('center').margin([0, 0, 0, 10]).build()
    )
    ...
    pdf.add(
      new Txt('Reporte de Afluencia simulada').alignment('center').bold().fontSize(15).margin([0, 10, 0, 1]).end
    );
