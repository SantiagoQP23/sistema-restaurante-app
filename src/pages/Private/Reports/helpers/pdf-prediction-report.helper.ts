...
export const generatePredictionReport = async (data: Footfall[], urlImage: string): Promise<ICreatePDF> => {
  ...
  PdfMakeWrapper.setFonts(pdfFonts);
    ...
    pdf.add(
      new Txt('Reporte de Predicción de afluencia').alignment('center').bold().fontSize(15).margin([0, 10, 0, 1]).end
    );
    ...
    const body = data.map((item) => {
      ...
      return [format(new Date(item.date), 'eeee dd MMMM yyyy', {locale: es}), item.quantity]
    }
    )
