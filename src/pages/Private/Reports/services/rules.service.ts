...
export const getRestaurant= async (restaurantId: string): Promise<Restaurant> =>{
  const resp = await restauranteApi.get<Restaurant>(`restaurant/${restaurantId}`);

  return resp.data; 
}


export const updateRestaurant = async  (restaurantId: string, restaurant: UpdateRestaurantDto): Promise<Restaurant> =>{
  const resp = await restauranteApi.patch<Restaurant>(`restaurant/${restaurantId}`, restaurant);
  return resp.data;
}
