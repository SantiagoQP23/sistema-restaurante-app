...
export const PdfAffluenceSimulation: FC<Props> = ({ days, month }) => {
    ...
    <Document  >
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
        ...
        <View>
          ...
          {
            month
              ? <Text style={{ fontSize: 12, marginBottom: 2 }}>Simulación de afluencia del mes {month}</Text>
        ...
        <View style={{ flexDirection: 'row', borderWidth: 1, borderColor: 'black' }}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.text}>Asistencia Simulada</Text>
          </View>
        </View>
        {days.map((day, index) => (
          <View key={index} style={{ flexDirection: 'row', borderWidth: 1, borderColor: 'black' }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.text}>{day.affluences.find(affluence => affluence.type === TypeAffluence["SIMULATED"])?.affluence}</Text>
            </View>
          </View>
        ))}
      </Page>
