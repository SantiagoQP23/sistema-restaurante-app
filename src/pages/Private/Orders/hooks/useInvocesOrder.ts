...
export const useCreateInvoiceOrder = () => {
  ...
  const createInvoiceOrder = (idOrder: CreateInvoiceDto) => {
    setLoading(true);
    socket?.emit(EventsEmitSocket.createInvoice, idOrder, (resp: SocketResponseOrder) => {
      if (resp.ok) {
        // dispatch(setActiveOrder(resp.order!))

        enqueueSnackbar('Factura creada correctamente', { variant: 'success' });
...
export const useUpdateInvoiceOrder = () => {
  ...
  const updateInvoiceOrder = (idOrder: UpdateInvoiceDto) => {
    setLoading(true);
    socket?.emit(EventsEmitSocket.updateInvoice, idOrder, (resp: SocketResponseOrder) => {
      if (resp.ok) {

        // dispatch(setActiveOrder(resp.order!))

        enqueueSnackbar('Factura actualizada correctamente', { variant: 'success' });
      } else {
        enqueueSnackbar(resp.msg, { variant: 'error' });
      }
      setLoading(false);
...
export const useDeleteInvoice = () => {
  ...
  const deleteInvoiceOrder = (deleteInvoice: UpdateInvoiceDto) => {
    setLoading(true);
    socket?.emit(EventsEmitSocket.deleteInvoice, deleteInvoice, (resp: SocketResponseOrder) => {
      if (resp.ok) {
